...
export interface ImportOptions extends DistroPlanOptions {
  ...
  dryRun?: boolean;
  /**
   * rootfs 的**兑底候选**（G22）：主源失败时依次尝试（都带同一个 sha256 —— 校验不可跳过）。
   * 顺序由 `core/netProfile ‹ rootfsCandidatesFor()` 给（CN 档 = NJU 主源 → Canonical 兑底）。
   */
  rootfsFallback?: { url: string; sha256: string }[];
  /** G22：apt 换源（只影响我们自建的发行版；不传 = 不动）。 */
  aptMirror?: AptMirrorRef;
}
...
/**
 * 取 rootfs：主源失败 → 依次试兑底候选（**每个都照旧过 sha256**），并明说换了源。
 *
 * §10 的两条纪律在这里落地：
 * - **不静默降级**：换源要打一行日志（用户看得到、可复盘）；
 * - **校验不可跳过**：候选可以有多个，sha256 只有一个（我们 pin 的那个）。
 */
async function acquireRootfs(opts: {
  primary: { url: string; sha256: string };
  fallback: readonly { url: string; sha256: string }[];
  cacheDir: string;
  bytes?: number;
  onProgress?: (p: RootfsProgress) => void;
  ...
  return last ?? { ok: false, reason: 'rootfs 没有可用候选' };
}
...
export async function importLaneDistro(opts: ImportOptions): Promise<ImportOutcome> {
  ...
  const script = laneBootstrapScript({
    ...
    // G22：CN 档在这里换来 apt 源（只改我们自建的发行版，先备份后改）
    ...(opts.aptMirror ? { aptMirror: opts.aptMirror } : {}),
  });
