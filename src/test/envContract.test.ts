...
describe('T06 envContract (single source of truth)', () => {
  ...
  it('a blocked lane also offers the EXPLICIT system fallback (never silent)', () => {
    ...
    // （旧 env 卡片的 HTML 断言随渲染器一起归档：`_archive/src/test/envCardSnapshot.test.ts`）
  ...
  it('the card renders every row, the baseline badge and the single next action', () => {
    ...
    // 契约本身要能看出"兼容模式"——旧卡片的 HTML 断言已随渲染器归档
    // （`_archive/src/test/envContract.test.ts` 保留了原版）
    const compilerRow = c.rows.find((r) => r.id === 'compiler');
    assert.strictEqual(compilerRow?.baseline, 'compatible', 'compatible baseline is recorded');
    assert.ok(c.rows.some((r) => r.id === 'lcov'), 'lcov row exists');
  });
