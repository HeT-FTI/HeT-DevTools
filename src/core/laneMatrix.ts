...
function posixLaneHeal(
  ...
  const rules: Record<LaneRequirement, HealRule> = {
    python: {
      kind: 'auto',
      evidence: [
  ...
  // ADR-8（2026-09-16）：Linux 主机的自愈能力取决于**免密 root**（WSL 侧恒有 `-u root`，
  // 平台原生免密）。矩阵是能力说明书，必须如实标注"自动装"是有条件的。
  if (exec === 'exec-linux') {
    for (const id of ['python', 'compiler', 'lcov', 'doxygen', 'graphviz', 'make'] as const) {
      rules[id].note += '（Linux：仅当免密 root 可用时自动安装；否则给出手动执行的 apt 命令，见 ADR-8）';
    }
  }
  return rules;
}
