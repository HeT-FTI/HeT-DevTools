...
import { aptMirrorLines, type AptMirrorRef } from './laneAptMirror';
...
export function laneBootstrapScript(input: {
  ...
  /** G22：apt 换源（只在**我们自建的发行版**里做；不传 = 不动源的现状）。 */
  ...
  return [
    ...
    conf.trimEnd(),
    'HET_WSL_CONF',
    // G22：apt 换源要在**第一次 apt-get update 之前**做（否则国内装包还是慢慢走官方源），
    // 只改我们自建的发行版；失败不阻断 bootstrap（换源是优化，不是前提）。
    ...aptMirrorLines(input.aptMirror),
