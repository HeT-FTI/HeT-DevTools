...
收尾说明：这一轮把 V2 计划（`workspace/develope/gui-v2-one-tab-plan.md` §6 的 A–K）全部落完，
逐块回执与"门禁已自证能失败"的证据在计划 §12；集成测试新增 `npm run test:c9`（真宿主一致性会话）。

### Added

- **页内细节视图（A 块）**：14 个细节面板收进驾驶舱页内的**互斥 Slot** —— 页签恒为 1，
  `het.detail.pin` 逃生门删除；门禁断言 `createWebviewPanel` 全仓库只剩驾驶舱一处。
- **Task 状态机（B 块）**：`core/tasks.ts`（穷举转换表 + 六条不变量 + 幂等键 + 并发上限 N=2 +
  心跳 10s + 取消真 abort + 重启恢复），`runWithBusy` 公开 API 不变（15 个动作调用点一行不改
  就获得 owner/deadline/心跳/确定出口），新增 `het.getTasks` / `het.task.cancel`。
- **状态项三档同源（C 块）**：`core/statusItem.ts` 把"一个状态"写成一个对象，芯片/悬停/页内
  只是它的三种投影；悬停收敛为 **5 条域行（行名 == rail 定名）+ ≤3 主动作 + ≤2 导航**，
  预算由 `hoverProjection` 强制。
- **输出收敛（D 块）**：Output 下拉里只剩 1 个 `HeT DevTools`；行格式收敛为
  `[hh:mm:ss] [domain] <glyph> text`；`log()` 必须给域；新增页内「输出」视图（环形缓冲 500 行
  + 域/级别/关键字过滤）。写通道只有三个入口（`log` / `logBlock` / `logStream`），裸写被门禁禁止。
- **Intent 注册表（I 块）**：`core/intents.ts` 把"动作 → 状态刷新/输出/prompt/视图/CI 对应"
  收成一张可校验的表（吸收原先四张平行表）；新增动作只需加一条。
- **任务中心（J 块）**：只读观测面板（在跑什么 / 跑过什么 / 结果如何）—— 唯一动作是取消；
  入口是 L1 忙点与悬停「打开」区的 `⏱ 任务`。
- **一致性会话（H 块）**：`npm run test:c9`（真宿主、0 人工、带看门狗）跑脚本化高频用户会话
  （6 段深链 + 14 个页内视图 + 52 次切换 + 长动作四出口），每步断言"任务 × 输出 × 前端"三面一致。
- **交叉编译一等动作（K 块）**：目标只来自 `.hetai/build-matrix.yml`；出目标架构包 + `readelf`
  架构报告；`-pr:b=default`（build context 隔离）与 `-tf=`（不跑测试）由门禁钉住；缺工具链时
  给"装哪个包 / 或者交给 CI"的定向提示。
- **上板验证（K 块）**：面板开头是「真机前置检查」（本次会不会刷写芯片 / 工具链 / 探针可见性 /
  触发开关 / 怎么办）；上板必须过确认门（自动化宿主视作取消 → 绝不静默刷写芯片）。

### Changed

- **术语与层级定稿（E 块）**：rail 五格（环境车道 / 构建验证 / 模块文档 / 质量安全 / 交付发布）
  + 齿轮「设置」（不进计数）；26 → 22 张卡（删掉重复的只读摘要段、新增交叉编译）；模块入口叫
  「AI 框架设计&实现」；HUD 整块删除（它并入悬停档）。
- **动作语义分清**：编译打包（本机包）≠ 交叉编译（目标架构，不跑测试）≠ 全量测试（与 CI 同链）
  ≠ 上板验证（真机）；卡片名与它那个按钮实际做的事对齐（走查修掉「运行预检 / 发布」这类名不副实）。
- `/het-setup` 预填带上 `[platform: cross]` 与 `[arch:]` 口径。

### Fixed

- **`het.task.deadlines` 设置只喂了执行层**：对账器仍按表里的默认值判超时 —— 用户照错误提示
  "调大设置"其实无效；现在两条路径读同一份覆盖值。
- **长动作进行中，状态栏 chip 不更新**（一直显示上一次的健康分，§F.35 想要的"进行中"从未到达）；
  现在"在跑什么"一变就重画（心跳/进度不算变化，避免闪烁）。
- **同一张悬停表里两种写法**：模块文档忙时说「构建中」、其它域说「进行中」→ 统一为一串字。
- **上板面板指向一个不存在的按钮**（文案写着"执行 ② 构建并上板"，但那个按钮从来没做）→ 补上。
- **`metadata.workflow_triggers.cross_compile` 在扩展的类型里缺失**（模板 schema 里它是必填）。
- **35 处 `channel.appendLine` 绕过结构化行与环形缓冲**（页内「输出」按域过滤会少一段）→
  全部收进三个写入口并加门禁。

### Removed

- `features/hud/**`（最后一个能自己开页签的地方）与它的 3 个测试文件、manifest 10 个命令 +
  10 个快捷键 + 2 项设置。
- 旧 UI 残留契约：`het.getCockpitState().page`、`het.detail.pin`、逐域输出通道名（`HeT DevTools · 构建` 这类）。

## [0.4.1] - 2026-09-20

### Added

- **单页驾驶舱成为唯一 UI（G1–G23）**：11 个 tab 收成「5 段工作流 + 竖向 rail + 吸顶状态条」，一页到底（滚动 > 点击 > 换页）；折叠状态记忆、正文懒加载、局部刷新。长耗时动作统一语义：转圈 + 禁用 + 输出通道有字 + 顶部忙点 + 完成通知 + 幂等（同一动作进行中重复点击直接忽略）。工具矩阵写清每个 `✗` 的「谁装 / 怎么装 / 要不要人」，「刷新状态」给时间戳。Copilot 入口（`/het-commit` `/het-docs` `/het-testgen` `/het-setup`）**只开一个会话**（禁用一切「新建会话」命令 + 忙语义去重），并尝试把会话命名为 `HeT DevTools Agent`。HUD 与单页共用同一套组件。
- **网络与源策略（`het.net.profile`）**：按 locale 自动选国内源（NJU 主源：pip/apt/rootfs），候选链失败自动退下一档；**不弹窗**，只在输出面板写一行决策记录。模板支持在线获取 v0.1.2（tarball + sha256 校验 + 缓存，无 git 也能建工程）。
...
- **旧 UI（多 tab 版）整条链路下线**：`het.ui.legacyTabs` 开关与中英文案一并移除——单页是唯一 UI，没有回退开关。被拿掉的代码**归档**在 `_archive/`（不在编译/测试范围内，`ARCHIVE.md` 记了每项的来源、替代物与还原方法）。
- **下载链路收口到 `core/fetch.ts`**：rootfs（340MB）与模板（3MB）共用同一条纪律——`.part` → 流式 sha256 → gzip 探针 → 原子改名；缓存命中也要重算 sha256（不做「信任缓存」的假设）。新增 `HET_ROOTFS_LIVE=1` 实网验收（默认跳过）。
...
- **单页驾驶舱的卡片全是 `—`**：事实原本只在旧「概览」页路径喂入，单页走不到 → 改为单页自己取事实，刷新经 `refreshGate`（不并发、不丢最新），打开/展开/动作后都会重填。
- **模板内的图片在仓库里是坏字节**：`.gitattributes` 的 `assets/template/** text eol=lf` 把 PNG/JPG 也当成文本做了行尾归一（`ALL_logo.jpg` 比上游少 1 字节，而它正是打进 vsix 的那份）→ 显式 `binary` 规则 + `git add --renormalize` 逐个复核。
- **自建 Windows 发行版的失败路径现在都说同一种话**：`wsl --import` 失败、发行版 bootstrap 失败、
  双标记校验失败，以及"发行版自建成功但车道起不来"，都给出同一组出路 —— A（装官方发行版
  `wsl --install -d Ubuntu-24.04`）或 C（改 `toolchain: system`），并明确"不会自动改用 MSVC"。
  此前只有 import 失败那一支给出路，其余只丢一段尾部输出（模块文档承诺过，代码没兑现）。
